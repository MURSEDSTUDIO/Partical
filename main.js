...
function getParticleCount(){
  ...
  if(isMobile)   return 300_000;   // Mobile: 300K — still insane
  if(isLowEnd)   return 800_000;   // Low-end desktop: 800K
  return 2_000_000;                // Beast desktop: 2M
  // Note: 5M requires a gaming GPU — set manually if needed
}
...
function buildLoader(){
  ...
  el.innerHTML = `
    ...
    <div class="axl-content">
      <div class="axl-title">AXTAR<span>GET</span></div>
      <div class="axl-subtitle">GESTURE PARTICLE SYSTEM</div>
      <div class="axl-subtitle2">WORLD EDITION</div>
...
async function main(){
  ...
  try {
    ...
    requestAnimationFrame(loop);
    console.log(`[AXTAR] 🔥 ${N.toLocaleString()} particles — World Edition active`);

  } catch(err){
    console.error('[AXTAR] Fatal error:', err);
