// ═══════════════════════════════════════════════════════════════
//  AXTAR — SHADERS.JS
//  World Champion GLSL — Particle + Post-Processing Pipeline
// ═══════════════════════════════════════════════════════════════
...
// ── Color palette: championship level ──
vec3 palette(float t, float speed, float dist){
  // Cyan core → electric blue → white hot tips
  vec3 a = vec3(0.02, 0.55, 0.85);
  vec3 b = vec3(0.30, 0.40, 0.45);
  vec3 c = vec3(1.00, 1.10, 0.90);
  vec3 d = vec3(0.00, 0.15, 0.55);
  vec3 base = a + b*cos(6.28318*(c*t + d));

  // Speed tint: fast=white-electric, slow=deep-cyan
  vec3 hot = vec3(0.6, 0.9, 1.0) + speed * vec3(0.5, 0.3, 0.1);
  return mix(base, hot, clamp(speed * 2.5, 0., 1.));
}
...
void main(){
  ...
  // Heat color: near hand = bright white-cyan
  vColor = mix(vColor, vec3(0.8, 1.0, 1.0), vTemp * vTemp * 0.7);
...
void main(){
  ...
  // White-hot core
  vec3 coreColor = mix(vColor, vec3(1.0,1.0,1.0), core*core*vTemp);
